...
class Diffuser:
    ...
    def get_loss(self, model, x_0, t):
        ...
        x_0_condition = x_0[:, :3, :, :]  # Flow parameters (first 3 channels)
        x_0_tragets = x_0[:, 3:, :, :]  # Last 3 airfoil flowfield channels
        x_noisy, noise = self.forward_diffusion(x_0_tragets, t)
        noise_pred = model(x_noisy, t, x_0_condition)
        return F.l1_loss(noise, noise_pred)

    def ddpm_sample_timestep(self, x, t,c, model):
        ...
        model_mean = sqrt_recip_alphas_t * (x - betas_t * model(x, t,c) / sqrt_one_minus_alphas_cumprod_t)
    ...
    def ddim_sample_timestep(self, x, t, c,model, eta=0.0):
    ...
    def sample_from_noise(self, model, condition, Tech="ddim"): 
        ...
        for i in timesteps:
            ...
            x_t = sampler(x_t, t_batch, c= condition, model=model)
